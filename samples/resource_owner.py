...
from okta_client.oauth2auth import ResourceOwnerFlow, StandardAuthenticationContext
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Resource Owner Password flow sample (not recommended for production).",
    )
    ...
    parser.add_argument("--redirect_uri", help="Redirect URI")
    ...
    parser.add_argument(
        "--test-config",
        dest="test_config",
        help="Path to test-configuration.json (username/password)",
    )
...
def main() -> None:
    ...
    print("WARNING: Resource Owner Password flow is not recommended for production use.")

    args = parser.parse_args()
    try:
        config = load_configuration(args)
        credentials = TestConfiguration(args.test_config, param_values=list(args.param or []))
    ...
    additional_parameters = credentials.additional_parameters()
    flow = ResourceOwnerFlow(client=oauth_client, additional_parameters=additional_parameters or None)

    username = credentials.get("username", "Username")
    password = credentials.get_secret("password", "Password")

    try:
        token = asyncio.run(flow.start(username, password, context=StandardAuthenticationContext()))
