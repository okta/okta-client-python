...
2. **`resume()`** _(optional)_ — completes a multi-step flow by exchanging the
   intermediate value for tokens (e.g., trading an authorization code or
   ID-JAG for an access token, polling for device authorization, etc.).

Single-step flows such as Resource Owner, JWT Bearer, and Refresh Token resolve
entirely in `start()`. Multi-step flows like Authorization Code and Cross-App
Access require a subsequent call to `resume()`.
...
`max_age`. See [Authentication Context](#authentication-context) below for
details.

### Authentication Context

All authentication flows accept an optional `context` parameter that carries
cross-cutting authentication properties. Some flows may require a specific
subclass (e.g., `AuthorizationCodeContext` for the Authorization Code flow)
which includes additional properties particular to that type of flow. Many
flows that do not have specific context requirements use `StandardAuthenticationContext`:

```python
from okta_client.authfoundation import StandardAuthenticationContext

context = StandardAuthenticationContext(
    audience="api://my-resource-server",
    resource="https://api.example.com/v1",  # or a list of URIs
    max_age=3600,
    nonce="custom-nonce",
)

token = await flow.start("jane@example.com", "password", context=context)
```

| Field | Type | Description |
| --- | --- | --- |
| `nonce` | `str \| None` | Custom nonce for ID token replay protection. |
| `max_age` | `float \| None` | Maximum authentication age (seconds). |
| `audience` | `str \| None` | Target resource server ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)). Sent in the token request. |
| `resource` | `str \| Sequence[str] \| None` | Target resource URI(s) ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)). Sent in the token request. |
| `acr_values` | `str \| None` | Requested Authentication Context Class Reference values. |
| `additional_parameters` | `Mapping[str, str] \| None` | Extra parameters forwarded to the token request. |
...
| `nonce` | Custom nonce value for ID token replay protection (auto-generated by default). |
| `audience` | Logical name of the target API / resource server ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)). Sent in the token request. |
| `resource` | Target resource URI(s) ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707)). Accepts a single string or a list of strings. Sent in the token request. |
